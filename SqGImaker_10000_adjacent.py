import os;import pymeshlab;import trimesh; from tqdm import tqdm;   
import pytorch3d.ops; import itertools; 
from pytorch3d.loss import chamfer_distance; 
import json; from sklearn.neighbors import KDTree; 
...
def pds(mesh_path,num_pts,output_pc_path): #poisson_disk_sampling
    ms_set=pymeshlab.MeshSet()
    ms_set.load_new_mesh(mesh_path)
    ms_set.generate_sampling_poisson_disk(samplenum=int(num_pts / (1 - 0.006)), exactnumflag=True) 
    pc = np.array(ms_set.current_mesh().vertex_matrix()) 
    index=np.random.permutation(pc.shape[0])[:num_pts] 
    pc=pc[index]
    np.savetxt(output_pc_path,pc)
...
def write_gi(save_path,points,H,W,normals=None):
    ...
    img.save(save_path) #for vscode gi view
...
def main_v1_1_seq(mesh_path, num_pts=10000, H=100, W=100, obj_l=[], pc_l=[], prior='./sphere.scale.pds_10000_structure_1step_20.xyz'):
    obj_l = obj_l
    pc_l = pc_l
    num_pts = num_pts
    H = H
    ...
    for i in range(len(pc_l)):
        sample_pc_path = pc_l[i]
        ...
        ext = 'xyz'
        # struc_pc_path = sample_pc_path.replace(f'.{ext}',f'_structure.xyz')
        # struc_gi_path = sample_pc_path.replace(f'.{ext}',f'_structure.png')
        refine_struc_pc_path = sample_pc_path.replace(f'.{ext}',f'_structure_refine.xyz')
        refine_struc_gi_path = sample_pc_path.replace(f'.{ext}',f'_structure_refine.png')
        # rearr_refine_struc_pc_path = sample_pc_path.replace(f'.{ext}',f'_structure_refine_rearr.xyz')
        # rearr_refine_struc_gi_path = sample_pc_path.replace(f'.{ext}',f'_structure_refine_rearr.png')
        save_metric_path = sample_pc_path.replace(f'.{ext}',f'_metric.json')

        metric = {}
        # metric['gi_bf_refine'] = {}
        metric['gi_af_refine'] = {}
        if i == 0: 
            first_frame = np.loadtxt(prior)
            gt_pc = np.loadtxt(sample_pc_path)
            this_frame = op_ssrf(first_frame, gt_pc, ep_rf)

            np.savetxt(refine_struc_pc_path, this_frame)

            write_gi(refine_struc_gi_path, this_frame, H=H, W=W, normals=None)
            cal_gi(H=H,W=W,metric_dict=metric['gi_af_refine'],points=this_frame,normals=None)
            metric['geo_af_refine'] = geo(torch.from_numpy(gt_pc).unsqueeze(0).cuda().float(), \
                                torch.from_numpy(this_frame).unsqueeze(0).cuda().float()).item()
...
if __name__ == '__main__':
    ...
    obj_l = []
    pc_l = []
    prior = './swing_pick_objs_sample1w_fpsUnify/0025.pds_10000_structure.xyz'
    for i in list(range(25,71,1)):
        number_str = "{:04d}".format(i)
        pc_l.append(f"./swing_pick_objs_sample1w_fpsUnify/{number_str}.xyz")
    main_v1_1_seq(mesh_path='', num_pts=10000, H=100, W=100, obj_l=obj_l, pc_l=pc_l, prior=prior)
